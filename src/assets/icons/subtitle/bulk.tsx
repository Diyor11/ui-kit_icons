import { FC } from "react";

interface IBulkProps {
  className?: string;
}

export const Bulk: FC<IBulkProps> = ({ className }) => {
  return (
    <svg
      className={className}
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M17.4999 17.83H15.6499C15.2399 17.83 14.8999 17.49 14.8999 17.08C14.8999 16.67 15.2399 16.33 15.6499 16.33H17.4999C17.9099 16.33 18.2499 16.67 18.2499 17.08C18.2499 17.49 17.9099 17.83 17.4999 17.83Z"
        fill="#292D32"
      />
      <path
        d="M12.97 17.83H6.5C6.09 17.83 5.75 17.49 5.75 17.08C5.75 16.67 6.09 16.33 6.5 16.33H12.97C13.38 16.33 13.72 16.67 13.72 17.08C13.72 17.49 13.39 17.83 12.97 17.83Z"
        fill="#292D32"
      />
      <path
        d="M17.5002 14.0699H11.9702C11.5602 14.0699 11.2202 13.7299 11.2202 13.3199C11.2202 12.9099 11.5602 12.5699 11.9702 12.5699H17.5002C17.9102 12.5699 18.2502 12.9099 18.2502 13.3199C18.2502 13.7299 17.9102 14.0699 17.5002 14.0699Z"
        fill="#292D32"
      />
      <path
        d="M9.27 14.0699H6.5C6.09 14.0699 5.75 13.7299 5.75 13.3199C5.75 12.9099 6.09 12.5699 6.5 12.5699H9.27C9.68 12.5699 10.02 12.9099 10.02 13.3199C10.02 13.7299 9.68 14.0699 9.27 14.0699Z"
        fill="#292D32"
      />
      <path
        opacity="0.4"
        d="M16.19 2H7.81C4.17 2 2 4.17 2 7.81V16.18C2 19.83 4.17 22 7.81 22H16.18C19.82 22 21.99 19.83 21.99 16.19V7.81C22 4.17 19.83 2 16.19 2Z"
        fill="#292D32"
      />
    </svg>
  );
};
