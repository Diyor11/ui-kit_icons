import { FC } from "react";

interface IBulkProps {
  className?: string;
}

export const Bulk: FC<IBulkProps> = ({ className }) => {
  return (
    <svg
      className={className}
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        opacity="0.4"
        d="M13 3.25H7C3.58 3.25 2.25 4.58 2.25 8V16C2.25 18.3 3.5 20.75 7 20.75H13C16.42 20.75 17.75 19.42 17.75 16V8C17.75 4.58 16.42 3.25 13 3.25Z"
        fill="#292D32"
      />
      <path
        d="M11.5001 11.38C12.5384 11.38 13.3801 10.5383 13.3801 9.5C13.3801 8.4617 12.5384 7.62 11.5001 7.62C10.4618 7.62 9.62012 8.4617 9.62012 9.5C9.62012 10.5383 10.4618 11.38 11.5001 11.38Z"
        fill="#292D32"
      />
      <path
        d="M21.65 6.16999C21.24 5.95999 20.38 5.71999 19.21 6.53999L17.73 7.57999C17.74 7.71999 17.75 7.84999 17.75 7.99999V16C17.75 16.15 17.73 16.28 17.73 16.42L19.21 17.46C19.83 17.9 20.37 18.04 20.8 18.04C21.17 18.04 21.46 17.94 21.65 17.84C22.06 17.63 22.75 17.06 22.75 15.63V8.37999C22.75 6.94999 22.06 6.37999 21.65 6.16999Z"
        fill="#292D32"
      />
    </svg>
  );
};
