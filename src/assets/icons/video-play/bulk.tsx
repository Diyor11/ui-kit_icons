import { FC } from "react";

interface IBulkProps {
  className?: string;
}

export const Bulk: FC<IBulkProps> = ({ className }) => {
  return (
    <svg
      className={className}
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        opacity="0.4"
        d="M22 7.81V16.19C22 19.83 19.83 22 16.19 22H7.81C4.17 22 2 19.83 2 16.19V7.81C2 7.3 2.04 6.81 2.13 6.36C2.64 3.61 4.67 2.01 7.77 2H16.23C19.33 2.01 21.36 3.61 21.87 6.36C21.96 6.81 22 7.3 22 7.81Z"
        fill="#292D32"
      />
      <path
        d="M22 7.81V7.86H2V7.81C2 7.3 2.04 6.81 2.13 6.36H7.77V2H9.27V6.36H14.73V2H16.23V6.36H21.87C21.96 6.81 22 7.3 22 7.81Z"
        fill="#292D32"
      />
      <path
        d="M14.4401 12.72L12.3601 11.52C11.5901 11.08 10.8501 11.02 10.2701 11.35C9.69012 11.68 9.37012 12.36 9.37012 13.24V15.64C9.37012 16.52 9.69012 17.2 10.2701 17.53C10.5201 17.67 10.8001 17.74 11.0901 17.74C11.4901 17.74 11.9201 17.61 12.3601 17.36L14.4401 16.16C15.2101 15.72 15.6301 15.1 15.6301 14.43C15.6301 13.76 15.2001 13.17 14.4401 12.72Z"
        fill="#292D32"
      />
    </svg>
  );
};
